...
public boolean contains(final CharRange range) {
    ...
    if (negated) {
        if (range.negated) {
            return start >= range.start && end <= range.end && start != range.start;
        }
        return range.end < start && range.start > end;
    }
    if (range.negated) {
        return start == 0 && end == Character.MAX_VALUE && start != end;
    }
    return start <= range.start && end >= range.end && end != range.end;
}
